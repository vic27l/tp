...
import { Save, ArrowLeft, User, Heart, Stethoscope, Smile, Activity, Baby, SmilePlus, Apple, Edit3 } from "lucide-react";
...
export default function EditarFicha() {
  ...
  const loadPaciente = async (id) => {
    try {
      ...
      if (paciente) {
        // Converter valores null para strings vazias ou valores padrão
        const formattedData = { ...paciente };
        
        // Campos de texto que devem ser strings vazias se null
        const textFields = [
        ...
        // Campos numéricos que devem ser strings vazias se null
        ...
        // Garantir que data_nascimento seja formatada corretamente
        if (formattedData.data_nascimento) {
          const date = new Date(formattedData.data_nascimento);
          formattedData.data_nascimento = date.toISOString().split('T')[0];
        } else {
          formattedData.data_nascimento = '';
        }
        
        // Garantir que mapa_dental seja um array
        if (!Array.isArray(formattedData.mapa_dental)) {
          formattedData.mapa_dental = [];
        }
  ...
  const loadConsultas = async (id) => {
    try {
      ...
      const consultasPaciente = todasConsultas.filter(c => c.paciente_id === id);
  ...
  const handleAddConsulta = async (consulta) => {
    try {
      const consultaPayload = {
        ...consulta,
        paciente_id: pacienteId,
        peso: Number(consulta.peso) || null,
        observacoes: consulta.observacoes || null,
        procedimentos: consulta.procedimentos || null
      };
      
      const novaConsulta = await Consulta.create(consultaPayload);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await Paciente.update(pacienteId, payload);
      
      alert('Ficha atualizada com sucesso!');
      // Redirecionar para visualizar a ficha
      navigate(`${createPageUrl("VisualizarFicha")}?id=${pacienteId}`);
    } catch (error) {
      console.error("Erro ao atualizar paciente:", error);
      
      // Mostrar erro mais detalhado
      let errorMessage = 'Erro ao atualizar a ficha. ';
      if (error.message) {
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-8">
          <div className="flex items-center space-x-4">
            <button
              onClick={() => navigate(`${createPageUrl("VisualizarFicha")}?id=${pacienteId}`)}
              className="glass-button p-3 rounded-lg text-white hover:bg-emerald-500/30 transition-all"
            >
              <ArrowLeft className="w-5 h-5" />
            </button>
            <div>
              <h1 className="text-3xl font-bold text-white">Editar Ficha</h1>
        ...
        <form onSubmit={handleSubmit} className="space-y-8">
          ...
          <div className="flex justify-center space-x-4">
            <button
              type="button"
              onClick={() => navigate(`${createPageUrl("VisualizarFicha")}?id=${pacienteId}`)}
              className="glass-button px-6 py-3 rounded-lg text-white font-semibold hover:bg-white/20 transition-all"
            >
              Cancelar
            </button>
            <button
